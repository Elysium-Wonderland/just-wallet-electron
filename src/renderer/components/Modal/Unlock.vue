<template>
    <modal title="Unlock" :visible="showDialog" @onCancel="close" class="JtTransfer">
        <div slot="content">
            <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" class="demo-ruleForm">
                ...
                <el-form-item>
                    <div class="btn" @click="transfer('ruleForm')">Unlock</div>
                </el-form-item>
            </el-form>
        </div>
    </modal>
                        ...
                        this.getAccount()
                            .then((account) => {
                                ...
                                if (address !== account.address) {
                                    this.$message.error(`Unlock failed: incorrect pin`);
                                    return;
                                }
                                return justjs.undelegate(account.address, toAddress, amount, mneString, justConstant.fee)
                            }).then(res => {
                            if (res && res.result === 'success') {
                                this.$message.success("Unlock success");
                                this.ruleForm = {
                                    address: '',
                                    count: '',
                                    code: ''
                                ...
                                this.$message.error(`Unlock failed: ` + res.error);
                            }
                        });
                    } else {
